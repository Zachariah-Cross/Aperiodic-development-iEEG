...
epoch_files = glob.glob('/Volumes/Zach_HD/ieeg_aperiodic/epoched_data/memory_development/*-epo.fif.gz')
...
##############################################################################
    ...
    for idx in range(data.shape[0]):
        ...
        fit_params['task'] = "memdev" # add task column
        dfs.append(fit_params) # append each epoch data frame
        
        # save IRASA output file for each subject
        # fit_params.to_csv('irasa_data/' + subj + '_aperiodic_memdev.csv', header = True)
        
        # # create larger IRASA data frame with each participant
        # df_export = pd.DataFrame(fit_params)
        # if op.isfile('aperiodic_memdev.csv'):
        #     df_export.to_csv('aperiodic_memdev.csv', sep=',', mode='a', header=False)
        # else:
        #     df_export.to_csv('aperiodic_memdev.csv', sep=',', mode='a', header=True)
        ...
        df_aperiodic_epoch['task'] = "memdev" # add task column
    ...
    # df_osc_df.to_csv('power_psd_data/' + subj + '_power_psd_memdev.csv', header = True)
    # df_aperiodic_df.to_csv('aperiodic_psd_data/' + subj + '_aperiodic_psd_memdev.csv', header = True)
    ...
    # plt.savefig('irasa_figures/' + subj + '_aperiodic_memdev.png',dpi = 300, 
    #             bbox_inches='tight');
    # plt.close();
    ...
    # plt.xlabel('Frequency [Hz]',fontsize = 20)
    # plt.ylabel('PSD log($uV^2$/Hz)',fontsize = 20)
    # plt.savefig('irasa_figures/' + subj + '_oscillatory_memdev.png',dpi = 300, 
    #             bbox_inches='tight');
    # plt.close();
...
##############################################################################
    ...
    for s in range(test.shape[0]): # shape refers to channel
        ...
        df_max['task'] = "memdev" # task name
        df_min['task'] = "memdev" # task name
        ...
        plt.xlabel('Frequency [Hz]', fontsize = 20)
        plt.ylabel('PSD log($uV^2$/Hz)', fontsize = 20)
        plt.savefig('peak_oscillations/' + subj + '_' + str(s) + '_oscillatory_memdev.png', 
                    dpi = 300, bbox_inches='tight');
        plt.close();
                
        # save output file for each subject
        df_max.to_csv('peak_data/' + subj + '_maxima_oscillations_memdev.csv', header = True)
        df_min.to_csv('peak_data/' + subj + '_minima_oscillations_memdev.csv', header = True)
        
    # save channel index information
    chan_information.to_csv('channel_information_memdev.csv', sep=',', mode='a', header=False)
